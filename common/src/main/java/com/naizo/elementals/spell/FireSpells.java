...
public final class FireSpells {
    ...
    public static void castSignature(Level level, Player player) {
        ...
        if (data.element == 1) {
            ...
            if (!level.isClientSide()) {
                player.addEffect(new MobEffectInstance(ModEffects.TIDEBINDER_EFFECT.get(), 100, (int) data.elementalLevel, false, false));
                data.cooldown = 300;
    ...
    public static void burnOnHit(Entity target, Entity owner) {
        ...
        target.setSecondsOnFire((int) ElementalsConfig.fire(level <= 1 ? "burn_timer_signature_level_0" : "burn_timer_signature_level_1"));
    }
